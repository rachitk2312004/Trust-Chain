import type { ScanCandidate, ScanCandidateType } from "../types/mobile.types";
...
const PATH_PATTERNS: Array<{ re: RegExp; type: ScanCandidateType; group: number }> = [
...
export function detectCandidates(
  ...
  for (const m of text.matchAll(VERIFY_CODE)) pushUnique(out, "verification_code", m[0]!, source);
  ...
  if (/^https?:\/\//i.test(text.trim())) pushUnique(out, "url", text.trim(), source);
  return out;
}
